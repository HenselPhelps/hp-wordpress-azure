...
add_filter("wpml_current_language", function($lang) {
    ...
    if (is_admin() && 'customize.php' === $pagenow) {
        ...
        if ($preview_url) {
            ...
            one_page_express_log2("wpml_current_language => $preview_url => $lang");
        }
    }
...
one_page_express_set_cookie_language();

add_action('init', function () {
    one_page_express_log2("--------------------------- init ----------------------------------");
    one_page_express_set_cookie_language();
});
...
function one_page_express_set_cookie_language()
{
    global $pagenow;
    if ('customize.php' === $pagenow) {
        global $wpml_request_handler;
        if ($wpml_request_handler) {
            one_page_express_log2("--------------------------- wpml_current_language => wpml_request_handler ----------------------------------");
            one_page_express_log2("lang1 == " . $wpml_request_handler->get_cookie_lang() . "=>" . one_page_express_get_current_language());
            $wpml_request_handler->set_language_cookie(one_page_express_get_current_language());
            one_page_express_log2("lang2 == " . $wpml_request_handler->get_cookie_lang() . "=>" . one_page_express_get_current_language());
        }
    }
}

function one_page_express_filter_nav_menu_locations($theme_locations)
{

    global $wpml_term_translations;

    if (is_admin() && (bool)$theme_locations === true) {
        $current_lang = one_page_express_get_current_language();
...
add_filter('theme_mod_nav_menu_locations', function ($theme_locations) {
    ...
    if (is_admin() && isset($pagenow) && $pagenow === 'customize.php') {
        global $one_page_express_menu_locations;
        $one_page_express_menu_locations = one_page_express_filter_nav_menu_locations($one_page_express_menu_locations);

        return $one_page_express_menu_locations;
    }
...
global $one_page_express_menus;
add_filter('wp_get_nav_menus', function ($menus) {
    global $pagenow;
    if (is_admin() && isset($pagenow) && $pagenow === 'customize.php') {
        global $one_page_express_menus;
        $one_page_express_menus = $menus;
    }
...
add_filter('wp_get_nav_menus', function ($menus) {
    ...
    if (is_admin() && isset($pagenow) && $pagenow === 'customize.php') {
        global $one_page_express_menus;
        $one_page_express_menus = one_page_express_filter_language_menus($one_page_express_menus);

        return $one_page_express_menus;
    }
...
function one_page_express_filter_language_menus($menus)
{
    global $sitepress, $wpml_term_translations;
    $current_language = one_page_express_get_current_language();
...
add_action('admin_footer', function () {
    ...
    if (strpos($current_screen->base, 'wpml-string-translation') === 0) {
        ...
        <script>
            (function ($) {
                $('option[value="admin_texts_theme_mods_one-page-express-pro"]').text("<?php _e('One Page Express Theme Texts', 'one-page-express-pro'); ?>");
            })(jQuery);
        </script>
        <?php
    }
...
function one_page_express_get_wpml_switcher($args = array(), $ulClass = "")
{
...
add_action('wp_footer', function () {
    ...
    if (function_exists('icl_get_languages')) {
        $content = one_page_express_get_wpml_switcher(array(), "{$position}-menu");
    }

    echo $content;
}, 10, 2);

function one_page_express_wpml_override_copy_content_function()
{
    ob_start();
...
add_action('admin_enqueue_scripts', 'one_page_express_enqueue_wpml_override_copy_content_function');

function one_page_express_enqueue_wpml_override_copy_content_function()
{
    wp_add_inline_script('sitepress-scripts', one_page_express_wpml_override_copy_content_function());
}